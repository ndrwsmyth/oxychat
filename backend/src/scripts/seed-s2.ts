...
interface ProjectDomainFixture {
  project_id: string;
  domain: string;
}

interface ProjectAliasFixture {
  project_id: string;
  alias: string;
}
...
interface SeedFixture {
  project_domains: ProjectDomainFixture[];
  project_aliases: ProjectAliasFixture[];
  transcripts: TranscriptFixture[];
}

function runSeedS1(): void {
  ...
  if (result.status !== 0) {
    throw new Error('seed:s1 failed');
  }
}

async function loadFixture(): Promise<SeedFixture> {
  const fixturePath = path.resolve(process.cwd(), 'seeds', 's2.fixture.json');
...
async function main() {
  ...
  if (fixture.project_domains.length > 0) {
    const { error } = await supabase
      .from('project_domains')
      .upsert(fixture.project_domains, { onConflict: 'project_id,normalized_domain' });
    if (error) {
      throw new Error(`Failed to seed project domains: ${error.message}`);
    }
  }

  if (fixture.project_aliases.length > 0) {
    const { error } = await supabase
      .from('project_aliases')
      .upsert(fixture.project_aliases, { onConflict: 'project_id,normalized_alias' });
    if (error) {
      throw new Error(`Failed to seed project aliases: ${error.message}`);
    }
  }
  ...
  if (transcriptError) {
    throw new Error(`Failed to seed transcripts: ${transcriptError.message}`);
  }
  ...
  if (attendeeRows.length > 0) {
    ...
    if (attendeeError) {
      throw new Error(`Failed to seed transcript attendees: ${attendeeError.message}`);
    }
  }
  ...
  if (classifiedTranscripts.length > 0) {
    const { error: classificationError } = await supabase
      ...
      .upsert(
        classifiedTranscripts.map((transcript) => ({
          ...
          external_attendee_count: transcript.attendees.filter(
            (attendee) => {
    ...
    if (classificationError) {
      throw new Error(`Failed to seed transcript classifications: ${classificationError.message}`);
    }
  }
  ...
  if (unclassifiedTranscriptIds.length > 0) {
    ...
    if (error) {
      throw new Error(`Failed to clear unclassified transcript rows: ${error.message}`);
    }
  }
  ...
  if (linkedTranscripts.length > 0) {
    ...
    if (linkError) {
      throw new Error(`Failed to seed transcript project links: ${linkError.message}`);
    }
  }
  ...
  if (unlinkedTranscriptIds.length > 0) {
    ...
    if (error) {
      throw new Error(`Failed to clear unlinked transcript rows: ${error.message}`);
    }
  }

  console.log('[seed:s2] Seeded Sprint 2 fixture successfully');
}

main().catch((error) => {
  console.error('[seed:s2] Failed:', error);
  process.exitCode = 1;
});
